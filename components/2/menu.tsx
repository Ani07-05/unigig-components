...
function MenuItem({ icon, active, href, onClick, children, className }: MenuItemProps) {
  ...
  const rowClasses = cn(
    "flex w-full items-center gap-3 rounded-md px-4 py-3 text-[14px] font-semibold transition-colors",
    active ? "bg-c-primary text-white" : "text-c-ink hover:bg-c-bg-2",
    className
  );
