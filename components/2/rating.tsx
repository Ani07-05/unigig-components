...
function Rating({ value, onChange, max = 5, readOnly = false, className }: RatingProps) {
  ...
  return (
    <div
      className={cn("inline-flex items-center gap-1 text-c-primary", className)}
    ...
    >
      {Array.from({ length: max }, (_, i) => i + 1).map((star) => (
        <button
          ...
          className={cn(
            "outline-none",
            !readOnly && "cursor-pointer focus-visible:ring-2 focus-visible:ring-c-primary focus-visible:ring-offset-2 focus-visible:ring-offset-c-bg"
          )}
