...
function Modal({ open, onOpenChange, children, className }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-c-ink/40 p-5">
      <div className="absolute inset-0" onClick={() => onOpenChange(false)} />
      <div
        role="dialog"
        aria-modal="true"
        className={cn(
          "relative w-full max-w-md rounded-lg border border-c-line bg-c-surface p-7 shadow-c",
          className
        )}
      >
        <button
          ...
          aria-label="Close"
          className="absolute right-5 top-5 flex h-8 w-8 items-center justify-center rounded-full border border-c-line bg-c-surface text-c-ink transition-colors hover:bg-c-primary hover:text-white"
        >
