...
const spinnerVariants = cva("inline-block animate-spin rounded-full border-c-primary border-t-transparent", {
  variants: {
    size: {
      sm: "h-4 w-4 border-2",
...
const ringVariants = cva("inline-block animate-spin rounded-full border-c-primary border-t-transparent", {
  variants: {
    size: {
    ...
    return (
      <span className={cn("inline-flex items-center gap-1.5", className)} {...props}>
        {[0, 1, 2].map((i) => (
          <span
            key={i}
            className={cn("animate-bounce rounded-full bg-c-primary", dotSizes[size])}
