...
function TabsList({ className, children, ...props }: TabsListProps) {
  ...
  return (
    <div
      ref={listRef}
      className={cn("relative flex items-center gap-6 border-b border-c-line", className)}
      {...props}
    >
      {children}
      {indicator && (
        <span
          aria-hidden
          className="absolute bottom-[-1px] h-[2px] bg-c-primary transition-all duration-200 ease-out"
...
function TabsTrigger({ value, className, children, ...props }: TabsTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "whitespace-nowrap py-3 font-corp-mono text-[12px] uppercase tracking-wide transition-colors",
        isActive ? "font-semibold text-c-ink" : "text-c-ink-soft hover:text-c-ink",
        className
      )}
      {...props}
    >
