...
function GigRow({ icon, title, meta, price, tag }: {
  icon?: React.ReactNode; title: string; meta: string; price: string; tag?: React.ReactNode;
}) {
  return (
    <Card className="flex items-center gap-4 px-4 py-3 shadow-[3px_3px_0_#000] transition-all hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[1px_1px_0_#000]">
      <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-[4px] border-2 border-black bg-[#ffe500] [&_svg]:h-[18px] [&_svg]:w-[18px]">
        {icon ?? <div className="h-2 w-2 rounded-none bg-black" />}
      </div>
...
function FeatureCard({ icon, tag, title, description, color = "#ffe500" }: {
  icon?: React.ReactNode; tag?: string; title: string; description: string; color?: string;
}) {
  return (
    <div className="rounded-[6px] border-2 border-black shadow-[4px_4px_0_#000] overflow-hidden">
      ...
      <div className="p-5 bg-white">
        {icon && (
          <div className="mb-2 flex h-8 w-8 items-center justify-center rounded-[4px] border-2 border-black bg-[#ffe500] [&_svg]:h-[18px] [&_svg]:w-[18px]">
            {icon}
          </div>
        )}
        {tag && (
